...
export async function discoveryParseHandler(req: Request, res: Response) {
  ...
  if (!openaiEnabled || !openai) {
    // Signal the client to use its deterministic local fallback.
    return res.json({ mode: "demo", reason: "OPENAI_API_KEY not configured" });
  }
  if (!transcript || typeof transcript !== "string" || !transcript.trim()) {
    return res.json({ mode: "demo", reason: "empty transcript" });
  }
  ...
  try {
    ...
    return res.json({ mode: "ai", discovery });
  } catch (e) {
    console.error("discovery parse failed:", (e as Error).message);
    return res.json({ mode: "demo", reason: "ai_error" });
  }
}
