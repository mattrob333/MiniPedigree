...
export interface DemoCompany {
  ...
  scenario?: "lumen" | "wesco";
}

const DEMOS: DemoCompany[] = [
  { file: "01_lumen_bay_startup_8_people.csv", label: "Lumen Bay", sub: "Startup · 8 people", kind: "guided", scenario: "lumen" },
  ...
  { file: "05_keystone_industrial_supply_12_people.csv", label: "Keystone Industrial Supply", sub: "AI governance pilot · 12 people", kind: "guided", scenario: "wesco" },
];
        ...
        <div className="home-grid">
          {DEMOS.map((d) => (
            <div key={d.file} className="home-card demo" onClick={async () => { setLoadingDemo(d.file); await onOpenDemo(d); setLoadingDemo(null); }}>
              <div className="hc-head"><div className="hc-name"><Icon name="csv" size={13} stroke="var(--cyan-dim)" style={{ verticalAlign: -2, marginRight: 6 }} />{d.label}</div>{d.kind === "guided" && <span className="tag cyan">{d.scenario === "wesco" ? "WESCO MVP" : "Guided demo"}</span>}</div>
