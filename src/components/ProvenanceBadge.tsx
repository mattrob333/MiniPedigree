...
export function ProvenanceBadge({ provenance, compact }: { provenance?: ItemProvenance; compact?: boolean }) {
  const [open, setOpen] = useState(false);
  const p = provenance ?? { state: "ai_inferred" as const };
  const style = STATE_STYLE[p.state];
  const tier = p.state === "ai_inferred" ? confidenceTier(p.confidence) : null;
  ...
  return (
    <span style={{ position: "relative", display: "inline-flex" }}>
      <button
        ...
        title={`${provenanceLabel(p.state)} — click for source`}
        style={{ cursor: "pointer", color: style.color, borderColor: style.color, background: "transparent", display: "inline-flex", alignItems: "center", gap: 4 }}
        aria-expanded={open}
      >
        <Icon name={style.icon} size={10} stroke={style.color} />
        {!compact && provenanceLabel(p.state)}
        {tier && !compact && <span style={{ opacity: 0.8 }}>· {tier}</span>}
      </button>
      {open && (
        <span
          style={{
            position: "absolute", top: "calc(100% + 6px)", right: 0, zIndex: 60, width: 300,
        ...
        >
          <span style={{ display: "flex", alignItems: "center", gap: 6, fontWeight: 600, color: style.color, marginBottom: 6 }}>
            <Icon name={style.icon} size={11} stroke={style.color} /> {provenanceLabel(p.state)}
            {p.confidence !== undefined && <span className="tag" style={{ marginLeft: "auto" }}>{Math.round(p.confidence * 100)}%</span>}
          </span>
          {p.evidence_quote ? (
            <span style={{ display: "block", fontStyle: "italic", color: "var(--text-2)" }}>"{p.evidence_quote}"</span>
          ) : p.state === "ai_inferred" ? (
